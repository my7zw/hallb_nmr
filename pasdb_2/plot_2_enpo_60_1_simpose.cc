...
int main(int argc, char **argv)
{
  ...
  TString nn_us = "2019-05-13_17h38m42s_area.root";
  TString nn_ub = "2019-05-13_16h53m26s_area.root";
  TString nn_ls = "QCV3640715679_area.root";
  TString nn_lb = "QCV3640711425_area.root";
  ...
  TH1Du *h_ub_df = (TH1Du*)h_ub->Clone("h_ub_df");//to get same #of bins for lanl
  Double_t offset= 0.0;
  ...
  for(Int_t i=0; i<h_ub_cl->GetNbinsX(); i++){
    ...
    h_ub_df->SetBinContent(i,(h_lb->GetBinContent(2*i)+h_lb->GetBinContent(2*i+1))/2.);
    //cout<<i<<" "<<h_ub->GetBinContent(i)<<" "<<h_lb->GetBinContent(i)<<endl;
  }
  h_ub_df->Add(h_ub, -1.);//h_ub_df became lanl minus uva
 
  h_ub_cl->Draw("HIST");
  h_ub_cl->GetXaxis()->SetLabelSize(0.055);
  h_ub_cl->GetXaxis()->SetNdivisions(504);
  h_ub_cl->GetYaxis()->SetLabelSize(0.055);
  h_ub_cl->GetYaxis()->SetNdivisions(405);
  h_ub_cl->GetYaxis()->SetRangeUser(-0.5,4.5);
  h_lb->Draw("HISTsame");
  ...
  for(Int_t i=0; i<h_ub_cl->GetNbinsX(); i++){
    ...
    h_us_df->SetBinContent(i,(h_ls->GetBinContent(2*i)+h_ls->GetBinContent(2*i+1))/2.);
  }
  h_us_df->Add(h_us, -1.);
    
  h_us->Draw("HIST");
  h_ls->Draw("HISTsame");
  h_us->GetXaxis()->SetLabelSize(0.055);
  h_us->GetXaxis()->SetNdivisions(504);
  h_us->GetYaxis()->SetLabelSize(0.055);
  h_us->GetYaxis()->SetNdivisions(405);
  h_us->GetYaxis()->SetRangeUser(-0.0007,0.0055);
  ...
  h_ls_bgr->GetXaxis()->SetNdivisions(504);
  h_ls_bgr->GetYaxis()->SetLabelSize(0.055);
  h_ls_bgr->GetYaxis()->SetNdivisions(405);
  h_ls_bgr->GetYaxis()->SetRangeUser(-0.0007,0.0055);
  ...
  h_lb_bgr->GetXaxis()->SetNdivisions(504);
  h_lb_bgr->GetYaxis()->SetLabelSize(0.055);
  h_lb_bgr->GetYaxis()->SetNdivisions(405);
  h_lb_bgr->GetYaxis()->SetRangeUser(-0.5,4.5);
  ...
  //c->SaveAs("pdf/lin_signals.pdf");
  c->SaveAs("pdf/lin_signals_upd.pdf");
  theApp->Run();  
